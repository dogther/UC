...
DAG_Gamma2* read_SHDL(string str_filename, uint32_t& in_num, uint32_t& out_num){
	...
	auto t1 = count(istreambuf_iterator<char>(file), istreambuf_iterator<char>(), 'o');
    file.clear();
    file.seekg(0, ios::beg);
	auto t2 = count(istreambuf_iterator<char>(file), istreambuf_iterator<char>(), 'r'); 
    file.clear();
    file.seekg(0, ios::beg);
	auto t3 = count(istreambuf_iterator<char>(file), istreambuf_iterator<char>(), 'g');
    file.clear();
    file.seekg(0, ios::beg);
	#ifdef DEBUG_READ_SHDL
	cout<<"o:"<<t1<<",r:"<<t2<<",g:"<<t3<<endl;
	#endif
	out_num = t1 - 1 - (t2 - t3);
    file.clear();
    file.seekg(0, ios::beg);
        ...
        while(getline(file, line)){
            ...
            if (line != "") {
                ...
                if(tokens[index + 1] == "gate"){
					...
					bool isL = false;
					if(tokens[index+4] == "operator") //add / mult gate
					{
						isL = true;
						c[0]=0; c[1]=0; c[2]=0; c[3]=0;
						if(tokens[index+5]=="+") {c[1]=1; c[2]=1;}
						else if(tokens[index+5]=="*") c[0]=1;
						else if(tokens[index+5]=="=") c[1]=1; //copy a
						else if(tokens[index+5]=="L") //linear function
						{
							//read 4 extra tokens
							c[0]=atoi(tokens[index+6].c_str());
							c[1]=atoi(tokens[index+7].c_str());
							c[2]=atoi(tokens[index+8].c_str());
							c[3]=atoi(tokens[index+9].c_str());
							index+=4;
						}
						input1 = g->node_array[atoi(tokens[index+8].c_str())];
						g->add_edge(input1,g->node_array[num]);
						if(arity==2)
						{
							input2 = g->node_array[atoi(tokens[index+9].c_str())];
		...
		while (getline(file, line)) {
			...
			if (line != "") {
				...
				if(tokens[index + 1] == "gate"){
					...
					{
						if(tokens[index+5]=="L") //linear
						{
							isL = true;
							for(int i=6; i<=9; i++)
								c[i] = atoi(tokens[i].c_str());
							index+=4;
							input1 = atoi(tokens[index+8].c_str());
							input2 = atoi(tokens[index+9].c_str());
							current = input1 * input2 * c[0] + input1 * c[1] + input2 * c[2] + c[3];
						}
						else
						{
						input1 = atoi(tokens[index+8].c_str());
						if(arity==2) input2 = atoi(tokens[index+9].c_str());
						if(tokens[index+5]=="+") function_number=6; //XOR
						else if(tokens[index+5]=="*") function_number=8; //AND
						else if(tokens[index+5]=="=") function_number=2; //COPY, arity 1
						}
					}
					else
					{
